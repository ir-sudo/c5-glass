...
(function () {
  ...
  if (!chat) return;
  const steps = [
    { type: "pill", html: "Missed call · just now", wait: 600 },
    { type: "biz", text: "Hi, thanks for calling [Your Shop] Glass! 👋 Sorry we missed you — we're out on a job. What glass work do you need? Text us right here and we'll jump on it.", wait: 900 },
    { type: "cust", text: "Need to replace all the windows in my house.", wait: 1400 },
    { type: "biz", text: "Happy to help with that. What city are you in?", wait: 1100 },
    { type: "cust", text: "San Jose — hoping to get it done this month.", wait: 1300 },
    { type: "biz", text: "Perfect. We'll text you a time to come measure within the hour. 🙌", wait: 1100 },
  ];
  const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
  const down = () => { chat.scrollTop = chat.scrollHeight; };
  const pill = (html) => { const e = document.createElement("div"); e.className = "chat__pill"; e.innerHTML = html; chat.appendChild(e); down(); };
  const bubble = (side, text) => { const e = document.createElement("div"); e.className = "bubble bubble--" + side; e.textContent = text; chat.appendChild(e); down(); };
  async function typing(ms) { const t = document.createElement("div"); t.className = "typing"; t.innerHTML = "<span></span><span></span><span></span>"; chat.appendChild(t); down(); await sleep(ms); t.remove(); }
  async function run() {
    while (true) {
      chat.innerHTML = "";
      for (const s of steps) {
        await sleep(s.wait);
        if (s.type === "pill") pill(s.html);
        else if (s.type === "biz") { await typing(1200); bubble("biz", s.text); }
        else bubble("cust", s.text);
      }
      await sleep(3200);
    }
  }
